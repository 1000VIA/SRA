           ...
           // var dato = $("#calendar .fc-celda-act").attr("data-date");
            ...
            $scope.Modificar = function () {
                ...
                if (ProgramacionSeleccionada.length == 1) {
                    ...
                    //    $("#ModalRegistroTransversal").modal("show");
                    //    $notify.info('Información', 'Seleccione dos horas entre las ' + ProgramacionSeleccionada[0].HoraInicio + ' y ' + ProgramacionSeleccionada[0].HoraFin);
                    //}
                    ...
                    bootbox.dialog({
                        title: "Información",
                        message: "Debe seleccionar una programación técnica de las que a continuación se listan y dar click en el boton <i class='material-icons' style ='margin-top:5px;'>&#xE89C;</i>",
                        buttons: {
                            success: {
                                label: "Cerrar",
                                className: "btn-primary",
            ...
            $scope.GuardarProgramacionTrasversal = function () {
                ...
                $scope.ProgramacionSeleccionada1.Resultado = $scope.Resultado.IdResultado

                if ($scope.ProgramacionSeleccionada1.IdResultado == "" || $scope.ProgramacionSeleccionada1.FechaFin == "") {
                    bootbox.dialog({
                        title: "Información",
                        message: "Debe diligenciar todos los campos",
                        buttons: {
                    ...
                    if (response.success == true) {
                        ProgramacionService.ConsultarProgramacion($rootScope.globals.currentUser.idpersona, function (response) {
                            if (response.success == true) {
                                ...
                                if (ProgramacionBorrar[0].DiaSemana != null) {
