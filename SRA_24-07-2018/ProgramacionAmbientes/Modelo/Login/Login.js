ProgramacionApp.controller('LoginController',
    ['$scope', '$rootScope', '$location', '$cookies', '$cookieStore', 'LoginService', '$routeParams', '$sce',
        function ($scope, $rootScope, $location, $cookies, $cookieStore, LoginService, $routeParams, $sce) {
            ...
            //PARAMETROS QUE EL USUARIO INGRESA EN EL LOGIN
            $scope.Usuario = {
                NombreUsuario: "",
                Password: ""
            };
            //PARAMETROS QUE SE UTILIZAN PARA RECUPERAR CONTRASEÑA
            ...
            //FUNCION PARA ABRIR MODAL DE RECUPERAR CONTRASEÑA
            $scope.AbrirModalRecuperar = function () {
                $("#ModalRecuperar").modal("show");
            };


            //FUNCIÓN QUE PERMITEEL INICIO DE SESION
            $scope.IniciarSesion = function () {
                ...
                LoginService.IniciarSesion($scope.Usuario, function (response) {
                    if (response.success == true) {
                        ...
                        debugger;
                        if ($rootScope.globals.currentUser.tipousuario == 4) { // Tipo Usuario Consulta
                            $(".items-menu-principal").css("display", "none");
                        }                        
                        if ($rootScope.globals.currentUser.tipousuario == 3) {  // Tipo Usuario Instrctor
                            $(".items-menu-principal").css("display", "none");
                            //$(".menuCalendario").addClass('active').hide();
                        }
                        ...
                        if (($rootScope.globals.currentUser.tipousuario == 5) && (Estado = true)) {
                            $(".not-admin").css("display", "none");
                        }
